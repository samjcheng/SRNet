...
from .update import *
from .net_util import AGGGate, nonlinearity
...
class IGEVMVSAGG(nn.Module):
    ...
    def forward(self, imgs, proj_matrices, depth_min, depth_max, test_mode=False):
        ...
        with autocast(enabled=self.mixed_precision):
            ...
            aux_init_feature = features[0][:, 0, :, :, :]
            # print("aux feature size", aux_init_feature.size())
            ref_feature = []
            for fea in features:
        ...
        with autocast(enabled=self.mixed_precision):
            ...
            aug_out = out
            # print("aux_out size", aug_out.size())

        return disp_init, disp_predictions, aug_out
